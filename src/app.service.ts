...
export class AppService {
  ...
  async saveStatements(text: string, cartaoId: number, cb: (progress: number) => void = () => { }) {
    ...
    try {
      ...
      const deleteResult = await this.prismaService.lancamentoCartao.deleteMany({
        where: {
          cartaoId,
          empresaId: 2,
          data: {
      ...
      const createResult = await this.prismaService.lancamentoCartao.createMany({
        data: dataToCreate,
      });
  ...
  transformToIInvoice(invoice: Invoice) {
    ...
    const result = invoice.transacoes.map((t, index) => {
